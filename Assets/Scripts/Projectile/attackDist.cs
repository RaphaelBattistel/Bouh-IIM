...
public class attackDist : MonoBehaviour
{
    ...
    public float projectilLifeTime = 5f;        // la durée de vie d'un projectile
    ...
    void Update() {
        ...
        if (Input.GetButton("Fire2") && !reloading) {
            ...
            projectilSave.GetComponent<Rigidbody2D>().linearVelocity = direction * speedProjectil;                    // Et on fait avancer le projectile dans la direction qu'on a calculé plutôt
